...
class SugarFieldFile extends SugarFieldBase
{
    ...
    {
        if(isset($vardef['allowEapm'])
                && $vardef['allowEapm'] == true)
        {
    ...
    public function save($bean, $params, $field, $properties, $prefix = '')
    {
        ...
        $this->fillInOptions($properties, $fakeDisplayParams);
        ...
        if ($move) {
            ...
            $upload_file->upload_doc($bean, $bean->id, $params[$prefix . $properties['docType']], $bean->$field, $upload_file->mime_type);
        } else if ( ! empty($old_id) ) {
            // It's a duplicate, I think

            if ( empty($params[$prefix . $properties['docUrl'] ]) ) {
            ...
            $displayParams = array();
            $this->fillInOptions($properties,$displayParams);

            if ( isset($params[$prefix . $properties['docId']])
                    && ! empty($params[$prefix . $properties['docId']])
                    && isset($params[$prefix . $properties['docType']]) 
                    && ! empty($params[$prefix . $properties['docType']])
               ) {
        ...
        if ( $properties['allowEapm'] == true && empty($bean->$field) ) {
            ...
            foreach ( $clearFields as $clearMe ) {
                if ( ! isset($properties[$clearMe]) ) {
                    continue;
                }
                $clearField = $properties[$clearMe];
                $bean->$clearField = '';
            }
        }
    }
