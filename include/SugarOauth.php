...
class SugarOAuth extends Zend_Oauth_Consumer
{
     ...
     */
    public function getOAuthToken($url, $callback = null, $params = array())
    {
        if(!empty($callback))
        {
        ...
        try
        {
            $this->_last = $token = $this->getRequestToken($params);
     ...
     * @return array
     */
    public function getOAuthAccessToken($url)
    {
        $this->setAccessTokenUrl($url);
        $this->_last = $token = $this->getAccessToken($_REQUEST, $this->makeRequestToken());
        return array('oauth_token' => $token->getToken(), 'oauth_token_secret' => $token->getTokenSecret());
    }
