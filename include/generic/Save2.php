...
elseif(isset($_REQUEST['return_type'])
...
elseif(isset($_REQUEST['return_type'])
    && $_REQUEST['return_type'] == 'addcampaignlog')
{
    // if param is set to "addcampaignlog", then we need to create a
    // campaign log entry for each campaign id passed in.

    // Get a list of campaigns selected.
    if(isset($_REQUEST['subpanel_id'])
    && !empty($_REQUEST['subpanel_id']))
    {
...
else
{
    ...
    && $_REQUEST['select_entire_list'] != 'undefined'
