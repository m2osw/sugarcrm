...
class SugarWidgetSubPanelTopComposeEmailButton extends SugarWidgetSubPanelTopButton
{
    ...
    {
        if((ACLController::moduleSupportsACL($layout_def['module'])  && !ACLController::checkAccess($layout_def['module'], 'edit', true) ||
                    $layout_def['module'] == "Activities" & !ACLController::checkAccess("Emails", 'edit', true))){
            $temp = '';
            return $temp;
        }
        ...
        $parent_type = $layout_def['focus']->module_dir;
        $parent_id = $layout_def['focus']->id;
        ...
        if($client != 'sugar') {
            ...
            $eUi = new EmailUI();
            $j_quickComposeOptions = $eUi->generateComposePackageForQuickCreate($composeData, http_build_query($composeData), false, $layout_def['focus']);
