...
class LayoutManager
{
    ...
    function getClassFromWidgetDef($widget_def, $use_default = false)
    {
        ...
        if(!empty($fieldDef)
        && !empty($fieldDef['type'])
        && strtolower(trim($fieldDef['type'])) == 'multienum')
        {
            $widget_def['widget_class'] = 'Fieldmultienum';
        }
        if(!empty($fieldDef)
        && !empty($fieldDef['type'])
        && strtolower(trim($fieldDef['type'])) == 'bool')
        {
    ...
    function widgetDisplay($widget_def, $use_default = false, $grabName = false, $grabId = false)
    {
        ...
        return $theclass->display($widget_def, null, false);
    }
