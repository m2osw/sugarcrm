...
class TemplateField
{
    ...
    function getDupMergeDomValue()
    {
        ...
        if(empty($this->merge_filter)
        || $this->merge_filter === 'disabled')
        {
            if(empty($this->duplicate_merge)
            || $this->duplicate_merge === 'disabled')
            {
    ...
    function populateFromPost()
    {
        foreach($this->vardef_map as $vardef => $field)
        {
            if(isset($_REQUEST[$vardef]))
            {
                ...
                if(is_string($this->$vardef)
                && in_array($vardef, $this->decode_from_request_fields_map))
                {
    ...
    function save($df)
    {
        ...
        if($this->unified_search
        && !isset($searchFieldParser->searchFields[$df->getModuleName()][$fieldName]))
        {
