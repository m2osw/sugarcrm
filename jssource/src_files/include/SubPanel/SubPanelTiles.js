...
function got_data(args, in_line)
{
...
function showSubPanel(child_field,url,force_load,layout_def_key)
{
  ...
  if (force_load || typeof( child_field_loaded[child_field] ) == 'undefined')
  {
    ...
    if ( typeof (url) == 'undefined' || url == null)
    {
      ...
      url = 'index.php?sugar_body_only=1&module='+module+'&subpanel='+child_field+'&action=SubPanelViewer&inline=' + in_line + '&record='+id + '&layout_def_key='+ layout_def_key;
    }
    ...
    var returnstuff = http_fetch_sync(url+ '&inline=' + in_line + '&ajaxSubpanel=true');
    request_id++;
    got_data(returnstuff, in_line);
  }
  else
  {
